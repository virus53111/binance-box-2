const CACHE='nexus-alpha-v8';
const STATIC=['./','./index.html','./styles-1.css','./styles-2.css','./styles-3.css','./app-1.js','./app-2.js','./app-3.js','./app-4.js','./app-5.js','./app-6.js','./app-7.js','./manifest.webmanifest','./icon.svg','./icon-192.png'];
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app-6.js')){
    event.respondWith(Promise.all([fetch(event.request),fetch(new URL('./app-7.js',self.location.href))]).then(async([main,guard])=>{
      if(!main.ok)return main;
      const body=(await main.text())+'\n'+(guard.ok?await guard.text():'');
      return new Response(body,{status:200,headers:{'content-type':'application/javascript; charset=utf-8','cache-control':'no-cache'}});
